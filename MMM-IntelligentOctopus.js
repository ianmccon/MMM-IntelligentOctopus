	...
	processDailyConsumption: function(raw) {
		...
		const peakConsumption = dataSet
			.map((el) => {
				...
				peakStartPeriod.setUTCHours(peakStartTime.split(":")[0], peakStartTime.split(":")[1], 0);
				const peakEndPeriod = new Date(el.interval_start)
				peakEndPeriod.setUTCHours(peakEndTime.split(":")[0], peakEndTime.split(":")[1], 0);
				const startTime = new Date(el.interval_start);
				const endTime = new Date(el.interval_end);

				if (endTime <= peakEndPeriod && startTime >= peakStartPeriod) {
